...
- **status**: todo — enhancement to the shipped party co-flight feature (branch `feature/party-coflight`, TASK not previously numbered - see `inc_flight.nss`)
- **action**: The cabin hatch currently always sends a follower to the pilot's *current* location (`FlightHatchToOwner`), and only falls back to the follower's stored boarding location (`FlightBoardLoc`) if the pilot has logged out. Give the follower an explicit choice: the hatch should ALWAYS be usable and offer both "join the pilot" and "return to my boarding location" — so a player can bail out of the flight and go back to where they boarded even while the owner is still piloting the ship. Likely a short conversation (or NUI) fired from `cabin_hatch.nss` instead of the current single unconditional jump; the boarding-location return path already exists (`FLIGHT_BOARD_LOC` local set in `FlightBoardParty`), it just needs to be selectable rather than logout-only. Keep the empty-cabin self-destroy cleanup working for both exit paths.
- **files**: `src/nss/inc_flight.nss` (`FlightHatchToOwner`, `FlightBoardParty`, `FLIGHT_BOARD_LOC`), `src/nss/cabin_hatch.nss` (hatch OnUsed), and the cabin areas `cabin_air000`/`cabin_star000` (`.git` hatch placeable) if a conversation resref needs wiring.
- **verify**: board a follower, take off; while the pilot is still aloft, use the hatch and choose "return to boarding location" — confirm the follower lands back where they boarded (not at the pilot), and choosing "join the pilot" still works. Confirm the cabin clone is destroyed once empty via either path.
