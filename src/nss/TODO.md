...
- **status**: todo
- **action**: A hired "mercenary ranger" henchman is repeatedly gaining extra copies of items it already has equipped; once its own inventory fills up the overflow lands in the PC's inventory instead. Find and fix whatever re-creates the same gear on top of what's already there.
- **files**:
  - `src/nss/henchs.nss` — action 14 ("Full equipment recall", ~line 340-420) is the strongest lead: it replays a saved inventory snapshot via `CreateItemOnObject(sItemBP,oHench,...)` in a loop with **no check that the hench doesn't already have that item** before recreating it. Confirmed this exact bug *class* elsewhere this session (`area_recall.nss`'s missing `iReady!=1` guard on `area_interests.nss`, fixed in commit `0bea260` — same shape: a content-recreation script with no re-entry guard, called from more than one place). Action 14 is triggered from three call sites — `henchs.nss:104`, `henchs.nss:311`, `soldiers_recall.nss:50` — worth checking whether any two of them can fire for the same hench close together (e.g. hire + a reactive gear-changed save both queuing it).
