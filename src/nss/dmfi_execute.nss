...
void DoVoiceFunction(int iSay, object oUser)
{
    ...
    if (!GetIsObjectValid(oTarget))
    {
        switch (iSay)
        {
            ...
            case 8: SetDMFIPersistentInt("dmfi", "dmfi_DMSpy", abs(GetDMFIPersistentInt("dmfi", "dmfi_DMSpy", oUser) - 1), oUser);
    ...
    else
    {
        switch (iSay)
        {
            ...
            default: sSay = GetDMFIPersistentString("dmfi", "hls206" + IntToString(iSay));
...
void DoRestFunction(int iRest, object oUser)
{
    ...
    switch (iRest)
    {
        ...
        case 34: //Set Time Limit to 8 game hours per day
        if (iCurrentArea & 0x00000080)
        {
            iCurrentArea = iCurrentArea & ~0x00000f00; //Erase current setting
        ...
        case 35: //Set Time Limit to 12 game hours per day
        if (iCurrentArea & 0x00000080)
        {
            iCurrentArea = iCurrentArea & ~0x00000f00; //Erase current setting
        ...
        case 36: //Set Time Limit to 24 game hours per day
        if (iCurrentArea & 0x00000080)
        {
            iCurrentArea = iCurrentArea & ~0x00000f00; //Erase current setting
