...
// Normally read back by inc_rand_equip.nss's EquipAdventurer() to bias gearing
// toward a ranged weapon; that system isn't ported here, but callers can still
// read this flag after AdvanceCreatureAlongAdventurerPath() to decide whether
// to auto-equip a ranged or melee weapon (see area_recall.nss).
...
// bEquip is accepted for signature compatibility but ignored - this module gears
// adventurers itself via its own treasure system (see area_recall.nss) instead of
// the ported inc_rand_equip/inc_treasure tiered-chest system.
...
int SelectAdventurerPath()
{
    ...
    i = 1;
    while (i <= ADVENTURER_PATH_HIGHEST)
    {
...
void AdvanceCreatureAlongAdventurerPath(object oCreature, int nPath, int nHD)
{
    ...
    // Head/skin/hair ranges match ~/git/gve3's g_os_good.nss randomiser.
    SetCreatureBodyPart(CREATURE_PART_HEAD, Random(11) + 1, oCreature);
    int nAdvSkin = Random(8);
    SetColor(oCreature, COLOR_CHANNEL_SKIN, nAdvSkin);
    if (nAdvSkin > 3)
    {
        SetColor(oCreature, COLOR_CHANNEL_HAIR, Random(8) + 15);
    }
    else
    {
        SetColor(oCreature, COLOR_CHANNEL_HAIR, Random(24) - 1);
    }
...
object SpawnAdventurer(location lSpawn, int nPath, int nHD, int bAdvance=1, int bEquip=1)
{
    ...
    if (bAdvance)
    {
        ...
        // Equipping is intentionally not handled here - callers gear the
        // creature themselves (this module uses its own treasure system
        // instead of the ported inc_rand_equip/inc_treasure tiered-chest system).
    }
    return oAdventurer;
}
