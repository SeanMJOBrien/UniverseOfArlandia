...
// fStartY, fStartAltitude) - oPla's real position is already its final
// landing spot, this is purely a client-side visual offset from that true
// position - then eases Y and Z back to 0 over fDurationSeconds, i.e. the
// hull appears to fly straight in along a fixed longitude (X never
// changes, starting from the edge of the area) and descend, rather than
// cutting in diagonally from a fixed corner - a straight single-axis
