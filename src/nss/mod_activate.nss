...
// A starship's own two areas count as its element too: a pilot who has gone
// below gets back to the helm through that same dialog, and with no way to open
// it down there they would be shut in. Those areas are not in ShipToolForArea,
// which also drives the ship-name rename (_shipname.nss) - aboard their own
// ship the PC is themselves again, not the ship model.
else if((sItemTag=="tool_ship")||(sItemTag=="tool_airship")||(sItemTag=="tool_starship")){if((ShipToolForArea(sAreaTag)==sItemTag)||((sItemTag=="tool_starship")&&(GetStringLeft(sAreaTag,6)=="pcship"))){SetLocalString(oPC,"shiptool",sItemTag);AssignCommand(oPC,ActionStartConversation(oPC,"ship",TRUE,FALSE));}else{ShipNameOpen(oPC,oItem);}}
////////////////////////////////////////////////////////////////////////////////
// Super power
