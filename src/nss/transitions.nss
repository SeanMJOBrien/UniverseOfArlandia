...
else{AssignCommand(oPC,ActionJumpToLocation(lLoc));if(GetLocalString(GetArea(oPC),"AreaExit")!=""){int iH=1;object oH=GetHenchman(oPC,iH);while(GetIsObjectValid(oH)){AssignCommand(oH,ClearAllActions(TRUE));AssignCommand(oH,ActionJumpToLocation(lLoc));iH++;oH=GetHenchman(oPC,iH);}}DeleteLocalInt(oPC,"Gate");}
  }
else if(iCheck==2)
  {
