...
// Use null coalescing operator to avoid warnings for undefined indexes
...
// Instead of querying the database inside the loop for every cell, we will fetch all
// relevant data for this planet/system into a cache array first. This reduces thousands
// of database queries into one, dramatically speeding up the page load.
$pwdata_cache = [];
$like_planet = $planet . '%';
$like_space = 'Space%';
$like_players = 'Player%';
$like_calendar = 'Calendar';
$like_reboot = 'Reboot';
$like_show = 'Show%';

$sql = "SELECT name, val FROM pwdata WHERE name LIKE ? OR name LIKE ? OR name LIKE ? OR name = ? OR name = ? OR name LIKE ?";
$stmt = mysqli_prepare($link, $sql);
mysqli_stmt_bind_param($stmt, "ssssss", $like_planet, $like_space, $like_players, $like_calendar, $like_reboot, $like_show);
...
// Fetch a single value from our new cache instead of the database.
...
<?php
$result = get_val_from_cache('Reboot');
if($result==""){$time1 = "offline";$time2 = $time1;}
else if($result=="rebooting"){$time1 = $result;$time2 = $time1;}
else{
...
<?php
$players = (int)get_val_from_cache('Players');
while($players>0){
    $player = get_val_from_cache('Player'.$players);
...
<?php
 ...
 {
echo "<br/>";?><center><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF><u><?php echo "$planet";?></u><?php echo "<br/><br/>";?><p><img border="0" src="<?php echo "$tiletype" ?>.jpg"></p><?php echo "<br/>";
 }
else
 {
echo "<br/>";
...
<table border="0" cellpadding="0" cellspacing="0">
  <tr>
   <td width="33%" align="center" valign="top"></td><td width="34%" align="center" valign="top"><a href="galaxy.php?planet=Space&login=<?php echo "$login" ?>&galaxyx=<?php echo "$galaxyx" ?>&galaxyy=<?php echo "$galaxyy"+1 ?>"><img border="0" src=Arrow_N.gif alt=North /></a></td><td width="33%" align="center" valign="top"></td>
  </tr><tr>
   <td width="33%" align="center" valign="top"><a href="galaxy.php?planet=Space&login=<?php echo "$login" ?>&galaxyx=<?php echo "$galaxyx"-1 ?>&galaxyy=<?php echo "$galaxyy" ?>"><img border="0" src=Arrow_W.gif alt=West /></a></td><td width="34%" align="center" valign="top"></td><td width="33%" align="center" valign="top"><a href="galaxy.php?planet=Space&login=<?php echo "$login" ?>&galaxyx=<?php echo "$galaxyx"+1 ?>&galaxyy=<?php echo "$galaxyy" ?>"><img border="0" src=Arrow_E.gif alt=East /></a></td>
  </tr><tr>
   <td width="33%" align="center" valign="top"></td><td width="34%" align="center" valign="top"><a href="galaxy.php?planet=Space&login=<?php echo "$login" ?>&galaxyx=<?php echo "$galaxyx" ?>&galaxyy=<?php echo "$galaxyy"-1 ?>"><img border="0" src=Arrow_S.gif alt=South /></a></td><td width="33%" align="center" valign="top"></td>
  </tr>
</table>
<?php
 } else {
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFFFFF" width="100%">
  <tr><td colspan="4"><p align="left"><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF><u>Local informations</u> :</font><p/>
<?php
$result = get_val_from_cache($planet);
...
?>
<p><font=Bell MT><font size=+1><font bold=True><font color=#00FFFF> <?php echo $descr; ?> </font><p/></td></tr>
 <tr>
   <td width="30%" valign="top"><br><p><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF>Position :</font><p/><br/><p><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF>Level :</font><p/><br/></td>
...
?>
<br><p><font=Bell MT><font size=+1><font bold=True><font color=#00FFFF><?php echo "$position" ?></font><p/><br/><p><font=Bell MT><font size=+1><font bold=True><font color=#00FFFF><?php echo "$level" ?></font><p/><br/></td>
    <td width="20%" valign="top"><br/><p><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF>Time :</font><p/><br/><p><font=Bell MT><font size=+1><font bold=True><font color=#66CCFF>Weather :</font><p/><br/></td>
    <td width="20%" valign="top">
<?php
...
if($weather=="0") {echo "<img src=weather_clear.gif alt=Clear />";} else if($weather=="1") {echo "<img src=weather_rain.gif alt=Rain />";} else if($weather=="2") {echo "<img src=weather_snow.gif alt=Snow />";} else if($weather=="3") {echo "<img src=weather_fog.gif alt=Fog />";} else if($weather=="4") {echo "<img src=weather_storm.gif alt=Storm />";}
?><br/></td></tr></table>
<?php
}
?>
...
if($Y==($size/2)+1) {
    ...
    while($X<$size/2+1) { ?><td bgcolor="#005064"><center><?php if($galaxyx!=0){echo $X+($galaxyx*15);}else{echo $X;} $X++; }
} else {
    $X = -($size/2)-1;
    while($X<$size/2+1) {
        if($X==-($size/2)-1) { ?><td bgcolor="#005064"><center><?php if($galaxyy!=0){echo $Y+($galaxyy*15);}else{echo $Y;}
        } else {
            if(($galaxyx!=0)&&($galaxyy!=0)){$XX = $X+($galaxyx*15);$YY = $Y+($galaxyy*15);} else if($galaxyx!=0){$XX = $X+($galaxyx*15);$YY = $Y;} else if($galaxyy!=0){$XX = $X;$YY = $Y+($galaxyy*15);} else{$XX = $X;$YY = $Y;}
            
            $planetarea = $planet."AreasX".$XX;
            $tile = get_val_from_cache($planetarea);
            ...
            if($planet!="Space") { if($tiletype=="01"){$tiletype = "clouds";} else if($tiletype=="02"){$tiletype = "desert";} else if($tiletype=="03"){$tiletype = "foothills";} else if($tiletype=="04"){$tiletype = "forest";} else if($tiletype=="05"){$tiletype = "frozenland";} else if($tiletype=="22"){$tiletype = "gaz";} else if($tiletype=="06"){$tiletype = "ground";} else if($tiletype=="07"){$tiletype = "hills";} else if($tiletype=="08"){$tiletype = "moon";} else if($tiletype=="09"){$tiletype = "mountain";} else if($tiletype=="10"){$tiletype = "mountsnow";} else if($tiletype=="11"){$tiletype = "mounts";} else if($tiletype=="12"){$tiletype = "ocean";} else if($tiletype=="13"){$tiletype = "plain";} else if($tiletype=="14"){$tiletype = "river";} else if($tiletype=="15"){$tiletype = "rural";} else if($tiletype=="16"){$tiletype = "ruralcastle";} else if($tiletype=="17"){$tiletype = "ruralswamp";} else if($tiletype=="18"){$tiletype = "seafloor";} else if($tiletype=="19"){$tiletype = "snow";} else if($tiletype=="20"){$tiletype = "swamp";} else if($tiletype=="21"){$tiletype = "tropical";} }

            ?><td width="24" height="24"><?php
            $area = str_replace("-", "m", $XX."_".$YY); 
            $interests = get_val_from_cache($planet."&".$area."&Interests");
            ...
            $planetshow = get_val_from_cache("Space".$area);
            ...
            $planetshow2 = get_val_from_cache("Space".$area."Show");
            
            $space = get_val_from_cache("Space".$area);
            $alt1 = str_replace("-", "m", $XX."_".$YY)." : ";
            ...
            if((($tiletype!="")&&(($login==$dmlogin)||($show==1)||($showAreas==1)||(($showInterests==1)&&($interest2!="")))||(($planet=="Space")&&($planetshow!="")&&(($login==$dmlogin)||($planetshow1==1)||($planetshow2==1))))) {
                ...
                else if($interest2=="D"){$alt3 = $alt1."Domain";$tiletype2 = $tiletype."_doma";} else if($interest2=="1"){$alt3 = $alt1.$name;$tiletype2 = $tiletype."_town";} else if($interest2=="2"){$alt3 = $alt1."Dungeon";$tiletype2 = $tiletype."_dung";} else if($interest2=="3"){$alt3 = $alt1."Castle";$tiletype2 = $tiletype."_cast";} else if($interest2=="4"){$alt3 = $alt1."Ruins";$tiletype2 = $tiletype."_ruin";} else if($interest2=="5"){$alt3 = $alt1."Animal reserve";$tiletype2 = $tiletype."_anre";} else if($interest2=="6"){$alt3 = $alt1."Resource mountain";$tiletype2 = $tiletype."_remo";} else if($interest2=="7"){$alt3 = $alt1."Amusement place";$tiletype2 = $tiletype."_amus";} else{$tiletype2 = $tiletype;}
